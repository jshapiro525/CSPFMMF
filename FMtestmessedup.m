...
lams = 1:.1:1.9;
parangs = 1:2:21;
[totalpert, injected, noise]=generatefakedatalam(lams,parangs, imdim,.002,17,epsilon);
...
fprintf('Finished unperturbed stuff\n\n')
...
fprintf('Finished perturbed stuff\n\n')
...
% 
% figure()
% plot(1:n,nU0,'b',1:n,nU,'r')
% legend('True Eigenvector norms','Estimated Eigenvector norms')
% title('Norms of the Eigenvectors')
% 
% figure()
% plot(nU0-nU)
% title('Difference in norms of Eigenvectors')

% figure()
% plot((nU0-nU)./nU0)
% title('Percent Difference in norms of Eigenvectors')
...
fprintf('Finished building cbar \n\n')
%% Checking results
myfig(Sh_un)
...
perdifcbar = reshape(perdifcbar,n^2,1);
pdifcbartoun = reshape(pdifcbartoun,n^2,1);

bins = linspace(0, ceil(max(perdifcbar)), ceil(max(perdifcbar)));
xc = histc(perdifcbar,bins);

figure()
semilogx(bins,xc,'r')
hold on
h = line([median(perdifcbar) median(perdifcbar)],[0 xc(2)]);
title('Distribution of percent error')
ylabel('Number of entries in Cbar')
xlabel('Percent Error')
legend('Distribution of Cbar Percent Error',horzcat(['Median = ' num2str(median(perdifcbar)) '%']))

bins = linspace(0, ceil(max(pdifcbartoun)), ceil(max(pdifcbartoun)));
xc = histc(pdifcbartoun,bins);

figure()
semilogx(bins,xc,'r')
hold on
h = line([median(pdifcbartoun) median(pdifcbartoun)],[0 xc(2)]);
title('Distribution of Cbar percent error to just the unperturbed model')
ylabel('Number of entries in Cbar')
xlabel('Percent Error')
